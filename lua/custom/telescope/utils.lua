...
local path_filename_first = function(path, reverse_directories, is_dir, dir_hl, coordinates)
  ...
  if coordinates:len() > 0 then
    local lineEnd = coordinates:find(":", 2)
    if lineEnd ~= nil then
      table.insert(path_style, { { #filename, #filename + lineEnd }, "TelescopeResultsComment" })
      -- table.insert(path_style, { { #filename + #lineEnd, #filename + #coordinates }, "Green" })
    end
  end
